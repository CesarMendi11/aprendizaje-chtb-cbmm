...
class CrawlJobExecutor:
    ...
    def execute(
        ...
        *,
        ...
        if normalized_scope not in {PipelineJobScope.FULL, PipelineJobScope.SCREEN}:
            raise CrawlJobExecutionError(
                "El runner de crawler sólo admite scope=full o scope=screen"
            )
        ...
        target_route = self._validate_target(profile, normalized_scope, target)
        ...
        with sync_playwright() as playwright:
            ...
            try:
                ...
                emit("logging_in", {"scope": normalized_scope.value, "target": target_route})
                navigator.login()
                emit(
                    "login_succeeded",
                    {
                ...
                summary = (
                    crawler.crawl_screen(target_route)
                    if normalized_scope == PipelineJobScope.SCREEN
                    else crawler.crawl()
                )
                return self._result(summary, run_root, normalized_scope, target_route)
            finally:
                try:
                    context.close()
                except Exception:
