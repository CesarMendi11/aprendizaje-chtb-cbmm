...
export default function App() {
  ...
  const [pipelineFocusJobId, setPipelineFocusJobId] = useState<string | null>(
    null,
  );
  const [tree, setTree] = useState<LoadState<KnowledgeTreeResponse>>({
    status: "loading",
  });
  const [detail, setDetail] =
    useState<LoadState<ScreenReviewContextResponse> | null>(null);
  ...
  const loadTree = useCallback(async () => {
    ...
    try {
      setTree({
        status: "ready",
        data: await getKnowledgeTree({ includeEmptyModules: true }),
      });
    } catch (error: unknown) {
      setTree((old) => ({
        status: "error",
  ...
  const loadDetail = useCallback(async (id: string) => {
    ...
    try {
      setDetail({ status: "ready", data: await getScreenReviewContext(id) });
    } catch (error: unknown) {
      setDetail((old) => ({
        status: "error",
        message: messageOf(error),
  ...
  const firstScreen = useMemo(
    () =>
      erp?.modules.flatMap((module) => module.screens)[0]?.screen_id ??
      erp?.unassigned_screens[0]?.screen_id ??
      null,
    [erp],
  );
  useEffect(() => {
    if (!selectedId && firstScreen) void loadDetail(firstScreen);
  }, [firstScreen, loadDetail, selectedId]);
  ...
  return (
    ...
    >
      ...
      {section === "structural" &&
        (knowledgePageProps ? (
          <StructuralKnowledgePage
            {...knowledgePageProps}
            onOpenJob={(jobId) => {
              setPipelineFocusJobId(jobId);
              setSection("pipeline");
            }}
          />
        ) : (
      ...
      {section === "semantic" &&
        (knowledgePageProps ? (
          <SemanticKnowledgePage {...knowledgePageProps} />
        ) : (
          <KnowledgeUnavailable state={tree} retry={loadTree} />
        ))}
      {section === "publication" && (
...
function KnowledgeUnavailable({
  ...
  state: LoadState<KnowledgeTreeResponse>;
  retry: () => void | Promise<void>;
}) {
  const message =
    state.status === "error"
      ? state.message
      : "La fuente de datos todavía no contiene un ERP activo con pantallas.";
  return (
