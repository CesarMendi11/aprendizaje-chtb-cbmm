...
const screenOptions = (erp: KnowledgeTreeErp | null): ScreenOption[] => {
  ...
  for (const screen of erp.unassigned_screens) {
    if (!screen.route || !screen.structural_available) continue
    options.push({ screen, label: `Sin módulo › ${screen.title ?? screen.route}` })
  }
  return options.sort((left, right) => left.label.localeCompare(right.label, 'es'))
}

export function CrawlTargetPicker({ disabled, onLaunch }: {
  disabled: boolean
  ...
  const erp = useMemo(
    () => tree?.erps.find((candidate) => candidate.erp_id === erpId) ?? tree?.erps[0] ?? null,
    [erpId, tree],
  )
  ...
  useEffect(() => {
    if (moduleId && !modules.some((module) => module.module_id === moduleId)) setModuleId('')
    if (screenId && !screens.some((option) => option.screen.screen_id === screenId)) setScreenId('')
  ...
  return <article className="pipeline-crawl-targets" aria-label="Recorridos parciales gobernados">
    ...
    {erp && <>
      <div className="pipeline-target-context">
        {tree && tree.erps.length > 1 && <label>
          <span>ERP</span>
          <select value={erp.erp_id} onChange={(event) => {
            setErpId(event.target.value)
            setModuleId('')
            setScreenId('')
          }} disabled={partialDisabled}>
            {tree.erps.map((candidate) => <option key={candidate.erp_id} value={candidate.erp_id}>{candidate.name}</option>)}
          </select>
        </label>}
        <div><span>ERP</span><strong>{erp.name}</strong></div>
        <div><span>ACTIVE</span><code>{erp.knowledge_version}</code></div>
      </div>
      ...
      <div className="pipeline-target-grid">
        <div className="pipeline-target-selector">
          ...
          <select id="pipeline-module-target" value={moduleId} onChange={(event) => setModuleId(event.target.value)} disabled={partialDisabled || modules.length === 0}>
            <option value="">Selecciona un módulo</option>
            {modules.map((module) => <option key={module.module_id} value={module.module_id}>{moduleLabel(module)}</option>)}
          </select>
          <small>{modules.length} módulos publicables disponibles.</small>
          <button onClick={launchModule} disabled={partialDisabled || !moduleId}>Recorrer módulo</button>
        </div>

        <div className="pipeline-target-selector">
          <label htmlFor="pipeline-screen-target">Pantalla</label>
          <select id="pipeline-screen-target" value={screenId} onChange={(event) => setScreenId(event.target.value)} disabled={partialDisabled || screens.length === 0}>
            <option value="">Selecciona una pantalla</option>
            {screens.map((option) => <option key={option.screen.screen_id} value={option.screen.screen_id}>{option.label}</option>)}
          </select>
          <small>{selectedScreen?.route ?? `${screens.length} pantallas con ruta canónica disponibles.`}</small>
          <button className="pipeline-primary" onClick={launchScreen} disabled={partialDisabled || !selectedScreen?.route}>Recorrer pantalla</button>
        </div>
      </div>
    </>}
  </article>
