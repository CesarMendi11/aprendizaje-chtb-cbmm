...
export function PipelineControl({ view = 'all', onOpenJob, focusJobId = null, refreshToken = 0 }: { view?: 'all' | 'pipeline' | 'publication'; onOpenJob?: (jobId: string) => void; focusJobId?: string | null; refreshToken?: number }) {
  ...
  const canMerge = Boolean(job && job.kind === 'canonical_build' && ['module', 'screen'].includes(job.scope) && job.status === 'succeeded' && !isBusy)
    ...
    <div className="pipeline-grid">
      <article className="pipeline-active">
        ...
        {!job ? <p className="pipeline-empty">Selecciona un target gobernado para un crawl parcial, ejecuta un FULL o abre un job reciente.</p> : <>
          ...
          {canMerge && <div className="pipeline-next-step"><span>Siguiente etapa disponible</span><button onClick={() => void launchCanonicalMerge()}>{job?.scope === 'screen' ? 'Fusionar esta pantalla con ACTIVE' : 'Fusionar este módulo con ACTIVE'}</button></div>}
