...
const entityLabel = (value: string) =>
  ({ ui_state: "Estado UI", event: "Evento", transition: "Transición" })[
    value
  ] ?? value;

const when = (value: string) => new Date(value).toLocaleString();

export function RemovalReviewConsole({
  onOpenJob,
}: {
  ...
  const loadImports = useCallback(async () => {
    ...
    try {
      ...
      setState((old) => ({
        ...
        imports,
        candidateVersionId:
          old.candidateVersionId ?? imports[0]?.knowledge_version_id ?? null,
        loading: false,
      }));
    } catch (error: unknown) {
      setState((old) => ({
    ...
    try {
      ...
      setState((old) => ({
        ...
        selectedDecisionId:
          ...
          review.decisions.some((item) => item.id === old.selectedDecisionId)
            ? old.selectedDecisionId
  ...
  const selectedDecision = useMemo(
    () =>
      state.review?.decisions.find(
        (item) => item.id === state.selectedDecisionId,
      ) ?? null,
  ...
  const act = async (action: "retain" | "remove" | "reset") => {
    ...
    try {
      if (action === "retain")
        await confirmRemovalRetain(selectedDecision.id, payload);
      else if (action === "remove")
        await confirmRemovalRemove(selectedDecision.id, payload);
  ...
  const reconcile = async () => {
    if (!state.review || state.review.pending_review !== 0 || state.submitting)
      return;
    setState((old) => ({ ...old, submitting: true, message: null }));
    try {
  ...
  return (
    <section
      className="removal-review"
      aria-label="Removal reconciliation HITL"
    >
      <div className="removal-review__heading">
        <div>
          ...
          <p>
            Una ausencia en un candidate FULL o parcial no prueba eliminación.
            Confirme retención o eliminación antes de construir el candidate
            reconciliado final.
          </p>
        </div>
        <button onClick={() => void loadImports()} disabled={state.loading}>
          Actualizar
        </button>
      </div>

      {state.message && (
        <div className="removal-message" role="alert">
          {state.message}
        </div>
      )}
      ...
      <div className="removal-toolbar">
          ...
          >
            ...
            {state.imports.map((job) => (
              <option key={job.id} value={job.knowledge_version_id ?? ""}>
                {job.knowledge_version_id?.slice(0, 8) ?? "sin versión"} ·{" "}
        ...
        {!state.review && (
          <button
            className="removal-primary"
            onClick={() => void prepare()}
            disabled={!state.candidateVersionId || state.submitting}
          >
            Preparar ReviewSet
          </button>
        )}
        {state.review && (
          <button
            ...
            disabled={state.review.pending_review !== 0 || state.submitting}
          >
      ...
      {state.review && (
        <>
          <div className="removal-counts">
            <div>
              <strong>{state.review.decision_count}</strong>
              <span>Total</span>
            </div>
            <div>
              <strong>{state.review.pending_review}</strong>
              <span>Pendientes</span>
            </div>
            <div>
              <strong>{state.review.retain_from_active}</strong>
              <span>Retain</span>
            </div>
            <div>
              <strong>{state.review.confirmed_remove}</strong>
              <span>Remove</span>
            </div>
          </div>
          ...
          <div className="removal-layout">
            <article className="removal-card removal-list">
              <header>
                <div>
                  ...
                  <h3>{state.review.candidate_knowledge_version}</h3>
                </div>
                <code>{state.review.candidate_origin}</code>
              </header>
              <div className="removal-items">
                {state.review.decisions.map((decision) => (
                  <button
                    key={decision.id}
                    className={
                      decision.id === selectedDecision?.id ? "is-selected" : ""
                    }
                    onClick={() =>
                      setState((old) => ({
                        ...old,
                  ...
                  >
                    <span>
                      <strong>{decision.canonical_id}</strong>
                      <small>
                        {entityLabel(decision.entity_type)} · rev.{" "}
                        {decision.review_revision}
                      </small>
                    </span>
                    <em
                      className={`removal-decision removal-decision--${decision.current_decision ?? "pending"}`}
                    >
                      {decisionLabel(decision.current_decision)}
                    </em>
                  </button>
                ))}
              </div>
            ...
            <article className="removal-card removal-detail">
              {!selectedDecision && (
                <div className="removal-empty">Seleccione una decisión.</div>
              )}
              {selectedDecision && (
                <>
                  <header>
                    <div>
                      <span>{entityLabel(selectedDecision.entity_type)}</span>
                      <h3>{selectedDecision.canonical_id}</h3>
                    </div>
                    <em
                      className={`removal-decision removal-decision--${selectedDecision.current_decision ?? "pending"}`}
                    >
                      {decisionLabel(selectedDecision.current_decision)}
                    </em>
                  </header>
                  <dl className="removal-meta">
                    <div>
                      <dt>Screen</dt>
                      <dd>{selectedDecision.screen_id ?? "sin screen"}</dd>
                    </div>
                    <div>
                      <dt>Plan</dt>
                      <dd>{selectedDecision.plan_reason}</dd>
                    </div>
                    <div>
                      <dt>Propuesta</dt>
                      <dd>{selectedDecision.proposed_decision}</dd>
                    </div>
                    <div>
                      <dt>Confirmación</dt>
                      <dd>{selectedDecision.removal_confirmation ?? "—"}</dd>
                    </div>
                  </dl>
                  ...
                  <div className="removal-form">
                    ...
                    <label>
                      ...
                      <textarea
                        ...
                        placeholder="Evidencia o criterio usado para decidir"
                      />
                    </label>
                    <div className="removal-actions">
                      <button
                        ...
                        disabled={
                          selectedDecision.current_decision !== null ||
                          !reviewerId.trim() ||
                          !reason.trim() ||
                          state.submitting
                        }
                  ...
                  {state.history && (
                    <div className="removal-history">
                      ...
                      {state.history.actions.length === 0 ? (
                        ...
                        <ol>
                          {state.history.actions.map((action) => (
                            <li key={action.id}>
                              <strong>{action.action}</strong>
                              <span>
                                {action.previous_decision ?? "pending"} →{" "}
                                {action.new_decision ?? "pending"}
                              </span>
                              <small>
                                {action.reviewer_subject} ·{" "}
                                {when(action.created_at)}
                              </small>
                              <p>{action.review_notes}</p>
                            </li>
                          ))}
                        </ol>
                      )}
