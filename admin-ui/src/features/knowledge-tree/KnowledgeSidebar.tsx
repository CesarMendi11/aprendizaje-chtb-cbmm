...
function buildModuleTree(
  ...
  for (const module of byId.values()) {
    const parent = module.parent_module_id
      ? byId.get(module.parent_module_id)
      : null;
  ...
  const sortNodes = (nodes: ModuleNode[]) => {
    nodes.sort((left, right) =>
      moduleLabel(left).localeCompare(moduleLabel(right), "es"),
    );
  ...
  const keepMatches = (nodes: ModuleNode[]): ModuleNode[] =>
    nodes.flatMap((node) => {
      const children = keepMatches(node.children);
      const moduleMatches = moduleLabel(node)
        .toLocaleLowerCase("es")
        .includes(needle);
      if (!moduleMatches && node.screens.length === 0 && children.length === 0)
        return [];
...
export function KnowledgeSidebar({
  ...
  const modules = useMemo(
    () => buildModuleTree(erp.modules, needle),
    [erp.modules, needle],
  );
  const unassigned = erp.unassigned_screens.filter((screen) =>
    screenMatches(screen, needle),
  );
  ...
  const screenButton = (screen: KnowledgeTreeScreen) => (
    ...
    >
      <span className="screen-title">
        {screen.title ?? "Pantalla sin título"}
      </span>
  ...
  const renderModule = (module: ModuleNode) => {
    ...
    return (
      <section className="tree-module" key={module.module_id}>
        <button
        ...
        {expanded && (
          <div className="tree-module__contents">
            {module.screens.length > 0 && (
              <div className="screen-list">
                {module.screens.map(screenButton)}
              </div>
            )}
            {module.children.length > 0 && (
              <div className="tree-module__children">
                {module.children.map(renderModule)}
              </div>
            )}
            {module.screens.length === 0 && module.children.length === 0 && (
              <EmptyState>
                Este módulo no contiene pantallas ni submódulos publicables.
              </EmptyState>
            )}
          </div>
        )}
  ...
  return (
    <aside className="sidebar" aria-label="Jerarquía de conocimiento">
      ...
      <label className="search">
        ...
        <div>
          ...
          <input
            ...
            placeholder="Título, módulo o ruta"
          />
        </div>
      </label>
      <nav aria-label="Módulos y pantallas">
        ...
        <section className="unassigned">
          ...
          {unassigned.length ? (
            unassigned.map(screenButton)
          ) : (
            <EmptyState>No hay pantallas sin asignar.</EmptyState>
          )}
        </section>
      </nav>
    </aside>
