...
export function PromotionGateConsole({
  ...
  const promote = async () => {
    ...
    try {
      const result = await promoteKnowledgeVersion(
        assessment.knowledge_version_id,
        {
  ...
  return (
    <section className="promotion-gate" aria-label="Promotion Gate">
      <div className="promotion-gate__heading">
        <div>
          <span>Promotion Gate</span>
          <h2>Activación gobernada de versión</h2>
          <p>
            Evalúa el candidate importado y, sólo cuando el gate esté listo,
            archiva la ACTIVE anterior y activa la nueva versión.
          </p>
        </div>
      ...
      {state.message && (
        <div className="promotion-message" role="alert">
          {state.message}
        </div>
      )}
      ...
      <div className="promotion-toolbar">
        ...
        {selectedImport && (
          <code className="promotion-job">
            import job {selectedImport.id.slice(0, 8)}
          </code>
        )}
      </div>
        ...
        <div className="promotion-layout">
          <article className="promotion-card">
            <header>
              <div>
                ...
                <h3>{assessment.knowledge_version}</h3>
              </div>
              <strong
                className={assessment.promotable ? "is-ready" : "is-blocked"}
              >
                {assessment.promotable ? "Promotable" : "Bloqueada"}
              </strong>
            </header>
            <div className="promotion-metrics">
              <div>
                <span>Modo</span>
                <strong>{assessment.promotion_mode}</strong>
              </div>
              <div>
                <span>Estado</span>
                <strong>{assessment.version_status}</strong>
              </div>
              <div>
                <span>ACTIVE actual</span>
                <strong>
                  {assessment.current_active_knowledge_version ?? "ninguna"}
                </strong>
              </div>
              <div>
                <span>Warnings</span>
                <strong>{assessment.build_warning_count}</strong>
              </div>
            </div>
            {assessment.diff_totals && (
              <div className="promotion-diff">
                {Object.entries(assessment.diff_totals).map(([key, value]) => (
                  <div key={key}>
                    <strong>{value}</strong>
                    <span>{key}</span>
                  </div>
            ...
            <div className="promotion-lineage">
              <span>Import</span>
              <code>{assessment.pipeline_import_job_id ?? "—"}</code>
              <span>Reconciliation</span>
              <code>{assessment.source_reconciliation_job_id ?? "—"}</code>
              <span>Removal review</span>
              <code>{assessment.removal_review_set_id ?? "—"}</code>
            </div>
          </article>
          ...
          <article className="promotion-card">
              ...
              <ul className="promotion-blockers">
                {assessment.blockers.map((blocker, index) => (
                  <li key={`${blocker.code}-${index}`}>
                    ...
                    <small>
                      {blocker.count}
                      {blocker.entity_type ? ` · ${blocker.entity_type}` : ""}
                    </small>
                  </li>
                ))}
              </ul>
            ...
            {assessment.warnings.length > 0 && (
              <div className="promotion-warnings">
      ...
      {state.result && (
        <div className="promotion-success">
          <strong>Versión activada</strong>
          <span>{state.result.knowledge_version}</span>
          <small>
            Promotion {state.result.promotion_id} · ACTIVE anterior{" "}
            {state.result.previous_active_version_id ?? "ninguna"}
          </small>
          <div>
            {Object.entries(state.result.sync_jobs).map(([target, jobId]) => (
              <code key={target}>
                {target}: {jobId}
              </code>
            ))}
