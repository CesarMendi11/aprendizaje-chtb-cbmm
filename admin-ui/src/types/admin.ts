export type ReviewStatus =
  ...
  | "corrected"
  | "rejected";
export type ScreenSemanticState =
  | "no_proposal"
  | ReviewStatus
  | "mixed"
  | "unavailable";
export type SemanticLifecycleOrigin =
  | "generated"
  | "carried_forward"
  | "reinferred";
...
export interface KnowledgeTreeScreen {
  screen_id: string;
  title: string | null;
  route: string | null;
  ...
  latest_semantic_id: string | null;
  latest_semantic_status: ReviewStatus | null;
  capabilities_count: number | null;
  evidence_available: boolean;
  warnings_count: number;
}
export interface KnowledgeTreeModule {
  module_id: string;
  ...
  diagnostic: string | null;
  order: number;
  screens: KnowledgeTreeScreen[];
  counters: AdminCounters;
}
export interface KnowledgeTreeErp {
  erp_id: string;
  name: string;
  slug: string;
  active_knowledge_version_id: string;
  knowledge_version: string;
  modules: KnowledgeTreeModule[];
  unassigned_screens: KnowledgeTreeScreen[];
  warnings: string[];
  counters: AdminCounters;
}
export interface KnowledgeTreeResponse {
  erps: KnowledgeTreeErp[];
}
export interface AdminScreenListResponse {
  items: {
    erp_id: string;
...
export interface NetworkTraceEvidence {
  evidence_id: string;
  methods: string[];
...
export interface ComparableStructure {
  screen_id: string | null;
  screen_title: string | null;
  screen_route: string | null;
  module: ModuleEvidence | null;
  fields: FieldEvidence[];
  controls: ControlEvidence[];
  tables: TableEvidence[];
  ui_states: UIStateEvidence[];
  events: EventEvidence[];
  transitions: TransitionEvidence[];
  network_traces: NetworkTraceEvidence[];
  evidence_ids: string[];
}
export interface StructuralEvidence extends ComparableStructure {
  evidence_available: boolean;
  diagnostic: string | null;
  screen_id: string;
  warnings: string[];
...
export interface ScreenPurposeInference {
  ...
  supported_capabilities: CapabilityClaim[];
  limitations: string[];
...
export interface AdminProposalSummary {
  ...
  source_semantic_proposal_id: string | null;
  source_knowledge_version_id: string | null;
  source_review_status: ReviewStatus | null;
  source_review_revision: number | null;
  source_effective_content_hash: string | null;
  erp_id: string;
  knowledge_version_id: string;
  screen_id: string;
...
export interface HistoricalProposalEvidence extends ComparableStructure {
  evidence_available: boolean;
  diagnostic: string | null;
  warnings: string[];
  evidence_hash: string;
...
export interface ProposalContext {
  ...
  effective_payload: ScreenPurposeInference | null;
  evidence: HistoricalProposalEvidence;
  historical_structure_hash: string | null;
  current_structure_hash: string;
  evidence_matches_current_structure: boolean;
  diagnostic: string | null;
}
...
export interface ScreenNavigation {
  ...
  module_screen_position: number;
  module_screen_total: number;
}
...
export interface ScreenReviewContextResponse {
  erp: { erp_id: string; name: string; slug: string };
  version: {
    knowledge_version_id: string;
    knowledge_version: string;
    status: string;
  ...
  semantic_proposals: ProposalContext[];
  active_proposal: ProposalContext | null;
...
export interface AdminSystemStatusResponse {
  ...
  services: {
    ...
    ollama: {
      ...
      configured_embedding_model_available?: boolean;
      models?: string[];
      detail?: string;
    };
  };
  knowledge: {
    ...
    latest_import: {
      id: string;
      status: string;
      requested_knowledge_version: string;
...
export type StructuralPublicationScope =
  | "screen"
  | "module"
  | "system"
  | "unscoped";
