...
def main(argv=None):
    ...
    parser.add_argument("--knowledge", default="data/processed/canonical/knowledge.json")
    args = parser.parse_args(argv)
    try:
        repo = CanonicalKnowledgeRepository(ROOT / args.knowledge)
    except Exception as exc:
        print(f"Error: {type(exc).__name__}: {exc}")
        return 1
    kb = repo.knowledge
    print(
        f"schema_version: {kb.schema_version}\nknowledge_version: {kb.knowledge_version}\nERP: {kb.erp_system.name} ({kb.erp_system.slug})"
    )
    for key, value in kb.statistics.items():
        print(f"{key}: {value}")
    pending = sum(
        getattr(item, "review_status", None) == "pending_review"
        for collection in (
            kb.modules,
            kb.screens,
            kb.ui_states,
    ...
    for module in kb.modules:
        print(f"\n[{module.name}]")
        for screen in repo.get_module_screens(module.id):
            print(f"  {screen.title} — {screen.route}")
    return 0
