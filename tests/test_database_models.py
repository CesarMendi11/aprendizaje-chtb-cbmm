...
def test_expected_database_tables_and_constraints():
    assert set(Base.metadata.tables) == {
        "erp_systems", "import_runs", "knowledge_versions",
        "knowledge_items", "review_actions", "sync_jobs",
    }
    item = Base.metadata.tables["knowledge_items"]
    assert {"canonical_id", "source_payload", "content_hash", "review_revision"} <= set(item.c.keys())
