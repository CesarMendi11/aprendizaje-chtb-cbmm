...
from erp_assistant.acquisition.crawling.frontier import Frontier
...
from erp_assistant.acquisition.crawling.state_frontier import StateFrontier
from erp_assistant.acquisition.crawling.state_registry import StateRegistry
from erp_assistant.acquisition.crawling.state_signature import StateSignature
...
def test_module_event_budget_is_relative_to_selected_module_depth():
    ...
    second_descendant = module_menu_state("#sales", "#tracking", "#integrations", "#providers")
...
def test_pinned_module_entry_records_route_revealed_by_final_expand():
    ...
    graph = crawler.routes_graph.to_dict()
    route_edges = [
        edge for edge in graph["edges"] if edge["kind"] == "module_scope_discovered_href"
    ]
